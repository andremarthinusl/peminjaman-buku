...
// Ambil data pengguna dengan pencarian dan paginasi
...
try {
    ...
    if(!empty($search)) {
        $count_sql .= " WHERE u.nama_lengkap LIKE :search 
                      OR u.username LIKE :search 
                      OR r.nama_role LIKE :search";
    }
    ...
    $total_pages = ceil($total_rows / $per_page);
      // Ambil data pengguna dengan paginasi dan pencarian
    $sql = "SELECT u.*, r.nama_role FROM users u
            JOIN roles r ON u.id_role = r.id_role";
    if(!empty($search)) {
        $sql .= " WHERE u.nama_lengkap LIKE :search 
                OR u.username LIKE :search 
                OR r.nama_role LIKE :search";
    }
    $sql .= " ORDER BY u.id_user ASC LIMIT :offset, :per_page";
...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-users-cog mr-2 text-indigo-600"></i> Manajemen Users
        </h1>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <form action="" method="GET" class="flex flex-col md:flex-row md:items-center">
            <div class="flex-grow mr-0 md:mr-4 mb-4 md:mb-0">
                <div class="relative">
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari user..." class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <?php if(!empty($search)): ?>
                    ...
                    <?php if(count($users) > 0): ?>
                        ...
                        <?php foreach($users as $user): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?= $user['id_role'] == 1 ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' ?>">
                                        <?= htmlspecialchars($user['nama_role']) ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="edit.php?id=<?= $user['id_user'] ?>" class="text-yellow-600 hover:text-yellow-900 mr-3">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <?php if($user['id_user'] != $_SESSION['user_id']): ?>
                                        <a href="javascript:void(0);" onclick="confirmDelete(<?= $user['id_user'] ?>)" class="text-red-600 hover:text-red-900">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                <?= !empty($search) ? 'Tidak ada hasil yang cocok dengan pencarian Anda.' : 'Belum ada data user.' ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
